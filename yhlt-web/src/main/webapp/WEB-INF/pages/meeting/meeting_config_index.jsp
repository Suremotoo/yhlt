	...
	<div id="container" class="easyui-layout layout" fit="true">
		<div region="north" border="false">
			<div id="queryHeader"
				...
				class="easyui-panel">
				<form class="form-horizontal" for="meetingConfigDatagrid">
		...
		<div region="center" border="false">
			<div style="padding: 5px; width: 100%;" class="easyui-layout layout"
				fit="true">
				<table id="meetingConfigDatagrid"></table>
			</div>
		</div>
	</div>
</div>
...
<div id="addWindow" class="easyui-dialog" title="新增"
	...
	<form class="addForm" action="${dynamicURL}/meeting/save"
		method="post">
...
<script type="text/javascript">
	//分页
	var meetingConfigDatagrid;
	$(function() {
		/*------------------初始化当前table 列表  datagrid 变量为全局变量-------------------*/
		meetingConfigDatagrid = $('#meetingConfigDatagrid').datagrid({
			url : '${dynamicURL}/meeting/list',
...
// 			fitcolumns: true,
			...
			onClickRow : function(rowIndex, rowData) {
				...
				if (radio1 != true) {
					...
					$("#meetingConfigDatagrid").datagrid("clearSelections");
	...
	$("#addWindow form.addForm").form({
		...
		success : function(data) {
			...
			if (data.success) {
				...
				meetingConfigDatagrid.datagrid('reload');
			} else {
	...
	//修改
	function edit() {
		var row = meetingConfigDatagrid.datagrid("getSelected");
		if (row) {
			var form = $("#addWindow form.addForm");
			form.form("clear");
			$.ajax({
				url : "${dynamicURL}/meeting/detail",
				data : {
					id : row.id
				},
				dataType : "json",
	...
	//删除
	function del() {
		top._del(meetingConfigDatagrid, {
			delUrl : "${dynamicURL}/meeting/delete"
		});
	}
</script>

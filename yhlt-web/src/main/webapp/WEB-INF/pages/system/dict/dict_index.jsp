...
<div id="addWindow" class="easyui-dialog" title="新增"
	...
	<form class="addForm" action="${dynamicURL}/system/dict/save"
		method="post">
...
<script type="text/javascript">
	...
	$(function() {
		...
		dictDatagrid = $('#dictDatagrid').datagrid({
			url : '${dynamicURL}/system/dict/list',
...
// 			fitcolumns: true,
	...
	$("#addWindow .BtnLine .submit").on("click", _.debounce(function() {
		if ($("#dictId").val() == null || $("#dictId").val() == "") {
			$.post('${dynamicURL}/system/dict/check', {
	...
	function edit() {
		...
		if (row) {
			...
			form.form("clear");
			$.ajax({
				url : "${dynamicURL}/system/dict/detail",
				data : {
					id : row.id
				},
				dataType : "json",
	...
	//删除
	function del() {
		top._del(dictDatagrid, {
			delUrl : "${dynamicURL}/system/dict/delete"
		});
	}
</script>

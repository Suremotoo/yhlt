        ...
        $members.each(function (index)
        {
            ...
            if (value && value.length > 0)
            {
                switch (datasource)
                {
                    ...
                    default:
                        $.ajax({
                            url:"Controls/SelectDict/GetNames@values=" + encodeURI(value), type: "get", async: false, cache: false, success: function (txt)
                            {
                                $_member.val(txt);
                            }
                        });
                    ...
                    case "1":
                        ...
                        $.ajax({
                            url:  "Controls/SelectDict/GetNames_SQL@values=" + encodeURI(value) + "&dbconn=" + dbconn + "&sql=" + encodeURI(sql), type: "get", async: false, cache: false, success: function (txt)
                            {
                                $_member.val(txt);
                            }
                        });
                    ...
                    case "3": //table
                        ...
                        url = "Controls/SelectDict/GetNames_Table@values=" + encodeURI(value);
            ...
            else
            {
                $but.bind("click", function ()
                {
                    ...
                    new RoadUI.Window().open({
                        id: "dict_" + id, url: "/Controls/SelectDict/Index?" + params, width: 500, height: 470, resize: false,
    ...
    this.setValue = function (objorid)
    {
        ...
        if (value && value.length > 0)
        {
            switch (datasource)
            {
                ...
                default:
                    $.ajax({
                        url: "Controls/SelectDict/GetNames@values=" + value, type: "get", async: false, cache: false, success: function (txt)
                        {
                            $obj.next().val(txt);
                        }
                    });

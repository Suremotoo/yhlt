...
; RoadUI.Member = function ()
{
    ...
    this.init = function ($members)
    {
        $members.each(function (index)
        {
            ...
            {
                $.ajax({
                    url: "Controls/SelectMember/GetNames@values=" + value, type: "get", async: false, cache: false, success: function (txt)
                    {
                        $_member.val(txt);
                    }
                });
            ...
            else
            {
                $but.bind("click", function ()
                {
                    ...
                    new RoadUI.Window().open({ id: "member_" + id, url:"/Controls/SelectMember/Index?" + params, width: 500, height: 470, resize: false, title: "选择组织机构成员", openerid: RoadUI.Core.query("tabid") || "" });
                });
            }

		...
		init : function(options) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					debug           : true,              	// Turn on voicewo debugging mode
				...
				if (flashInstalled) {
					...
					swfobject.embedSWF(settings.swf, settings.id + '-swf', settings.width, settings.height, '9.0.0', null, {id: settings.id, debug: settings.debug}, {wmode: 'transparent', menu: 'true', allowScriptAccess: 'always', quality: 'high'}, {id: settings.id + '-swf', name: settings.id + '-swf', styleclass: settings.id + '-swf', style: 'position: absolute; z-index:99999'});
					$swf = $('#' + settings.id + '-swf');
